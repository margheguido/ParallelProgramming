...
*/
void cgsolver( double *A, double *b, double *x, int m, int n , MPI_Comm mpicommunicator, int * m_ranks, int * displ_ranks){
	double * r;
	double * p;
	double rsold;
	double rsnew;
	...
	// m_loc of this rank is obtained from the rank thet contains it for all processors 
	m_loc = m_ranks[rank];
	int lda = m_loc;
...
//  r = b - A * x;
	...
	// Gather the result in Ap for all processors 
...
//      Ap = A * p;
		...
		// Everyone compute the following calculation
//      alpha = rsold / (p' * Ap);
...
//      rsold = rsnew;
	...
	// Local result of the final residual 
	memset(r, 0., n * sizeof(double));
	// Local matrix-vector 
	cblas_dgemv (CblasColMajor, CblasNoTrans, m_loc, n, al, A, lda, x, 1, be, r_loc, 1);
	// All gather the result for final residual
	MPI_Allgatherv(r_loc,m_loc, MPI_DOUBLE,r, m_ranks,displ_ranks,MPI_DOUBLE, MPI_COMM_WORLD);
